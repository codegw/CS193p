...
//  Memorize
//
//  Created by codegw on 08/07/2024.
//
...
struct MemoryGame<CardContent> where CardContent: Equatable {
    ...
    init(theme: Theme, cardContentFactory: (Int) -> CardContent) {
        self.theme = theme
        cards = []
        for pairIndex in 0..<min(theme.numOfPairs, theme.emoji.count) {
            ...
            if !cards[chosenIndex].isFaceUp && !cards[chosenIndex].isMatched {
                if let potentialMatchIndex = indexOfTheOneAndOnlyFaceUpCard {
                    if cards[chosenIndex].content == cards[potentialMatchIndex].content {
                        ...
                        gameScore += 2 + cards[chosenIndex].bonus + cards[potentialMatchIndex].bonus
                    }
                    else {
    ...
    struct Card: Equatable, Identifiable, CustomDebugStringConvertible {
        ...
        var debugDescription: String {
            "\(id): \(content): \(isFaceUp ? "up" : "down") \(isMatched ? "matched" : "") \(isSeen ? "yes" : "no" )"
        }
        // MARK: - Bonus Time
